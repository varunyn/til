...
export default function Tweet({
  ...
  return (
    <div className="tweet rounded-lg border border-gray-300 dark:border-gray-800 px-4 sm:px-6 py-4 my-4 w-full">
      <div className="flex items-center">
        <a
          className="flex h-10 w-10 sm:h-12 sm:w-12 flex-shrink-0"
          href={authorUrl}
          target="_blank"
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="author flex flex-col ml-3 sm:ml-4 !no-underline overflow-hidden"
        >
          <span
            className="flex items-center font-bold !text-gray-900 dark:!text-gray-100 leading-5 text-sm sm:text-base truncate"
            title={author.name}
          >
            {author.name}
            {author.verified ? (
      ...
      <div className="mt-3 mb-1 leading-normal whitespace-pre-wrap text-base sm:text-lg !text-gray-700 dark:!text-gray-300">
        {formattedText}
      </div>
      {media && media.length ? (
        <div
          className={
            media.length === 1
              ? 'inline-grid grid-cols-1 gap-x-2 gap-y-2 my-2 w-full'
              : 'inline-grid grid-cols-1 sm:grid-cols-2 gap-x-2 gap-y-2 my-2 w-full'
          }
        >
          {media.map((m) => (
            <img
      ...
      <a
        className="!text-gray-500 text-xs sm:text-sm hover:!underline block"
        href={tweetUrl}
        target="_blank"
        rel="noopener noreferrer"
      >
